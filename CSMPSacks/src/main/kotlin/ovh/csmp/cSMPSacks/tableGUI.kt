...
class tableGUI: Listener {
    ...
    private fun buyFgLeather(player: Player) {
        ...
        if (count < 16) {
            ...
            bugs.instance.removeItem(player, ItemStack(Material.LEATHER, 16))
    ...
    private fun buyRLeather(player: Player) {
        ...
        if (count < 16) {
            ...
            bugs.instance.removeItem(player, items.full_grain_leather.returnItem(16))
    ...
    private fun sacksGUI(player: Player) {
        ...
        imeta.lore = listOf("§b64x Full-grain Leather", "§bIllusion Cloak", "1x Travellers' Sack")
    ...
    private fun buyTSack(player: Player) {
        ...
        if (fgl < 16 || mgw < 1) {
            ...
            bugs.instance.removeItem(player, items.full_grain_leather.returnItem(16))
            bugs.instance.removeItem(player, items.magic_wand.returnItem())
            ...
            listener.instance.expUpdate(player)
        }
    }
    ...
    private fun buyISack(player: Player) {
        ...
        val tvs: Int = getItemCount(player, sacks.travellers_sack.returnSack())
        if (fgl < 64 || ic < 1 || tvs < 1) {
            player.sendMessage("§4You dont have enought items for this craft")
        } else {
            bugs.instance.removeItem(player, items.full_grain_leather.returnItem(64))
            bugs.instance.removeItem(player, items.illusion_cloak.returnItem())
            bugs.instance.removeItem(player, sacks.travellers_sack.returnSack())
